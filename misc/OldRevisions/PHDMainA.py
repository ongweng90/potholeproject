...
state=0
RACount=10
count1=100    #take a distance sensor measurement every count1 accel measurements
count2=1000   #take a gps every count2 accel measurements
x=1
x2=0
x2Max=100
trigger=0
...
while (time.time() - start <= 1000):
    ...
    if (state==1):
        if (x%1000==1):
            fileName="GtestFast" + time.strftime('%H%M%S') + ".txt"
    ...
    if (state==2):
        ...
        dist=PHDLibraries.captureDistance()
        file.write(str(x)+" "+str(z)+" "+str(dist)+" "+str(trigger))
        if ((dist<distTriggerLow)|(dist>distTriggerHigh)|(x2>x2Max)):
